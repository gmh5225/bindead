...
public class Elf {
	...
	public class Section {
		...
		/* sh_flags */
                ...
                public final static int SHF_EXECINTR = 0x4;
                public final static int SHF_MERGE = 0x10;
                public final static int SHF_STRINGS = 0x20;
                public final static int SHF_INFO_LINK = 0x40;
                public final static int SHF_LINK_ORDER = 0x80;
                public final static int SHF_OS_NONCONFORMING = 0x100;
                public final static int SHF_GROUP = 0x200;
                public final static int SHF_TLS = 0x400;
                public final static int SHF_MASKOS = 0x0ff00000;
		...
		public byte[] loadSectionData() throws IOException {
                  ...
                  if (sh_type == SHT_NOBITS) {
                    // do not try to read from sections that are not present as data in the file
    ...
    /**
     * Read a 32 or 64 bit chunk from the given offset into this section.
     */
    public long readChunkAtOffset (long offset) throws IOException {
      efile.seek(sh_offset + offset);
      switch (ehdr.e_ident[ELFhdr.EI_CLASS]) {
      case ELFhdr.ELFCLASS32: {
        return efile.readIntE();
      }
      case ELFhdr.ELFCLASS64: {
        return readUnsignedLong(efile);
      }
      case ELFhdr.ELFCLASSNONE:
      default:
    ...
    public byte[] loadSymbolData () throws IOException {
      Section containingSection = getSections()[st_shndx];
      long offsetInSection = containingSection.sh_addr.distanceTo(st_value).longValue();
      long offsetInFile = containingSection.sh_offset + offsetInSection;
      if (st_size <= 0 || offsetInFile < 0)
        return new byte[0];
      byte[] data = new byte[(int) st_size];
      efile.seek(offsetInFile);
      efile.read(data);
      return data;
    }
  ...
  public static class Rela_Relocation {
  ...
  public List<Rela_Relocation> getPLTRelocations (Section pltRelocations) throws IOException {
    ArrayList<Rela_Relocation> relocations = new ArrayList<Rela_Relocation>();
    if (pltRelocations.sh_type != Section.SHT_RELA && pltRelocations.sh_type != Section.SHT_REL)
      return relocations;
    int RELOC_ENT_SIZE_32 = 8;
    int RELOC_ENT_SIZE_64 = 16;
    boolean hasAddend = pltRelocations.sh_type == Section.SHT_RELA;
    if (hasAddend) {
      RELOC_ENT_SIZE_32 = 12;
      RELOC_ENT_SIZE_64 = 24;
    }
    efile.seek(pltRelocations.sh_offset);
    int off = 0;
    while (off < pltRelocations.sh_size) {
      Rela_Relocation relocation = new Rela_Relocation();
      switch (ehdr.e_ident[ELFhdr.EI_CLASS]) {
      case ELFhdr.ELFCLASS32: {
        relocation.r_offset = efile.readIntE();
        relocation.r_info = efile.readIntE();
        if (hasAddend)
          relocation.r_addend = efile.readIntE();
        relocation.relocationType = (int) (relocation.r_info & 0xff);
        relocation.symbolTableIndex = (int) (relocation.r_info >> 8);
        off += RELOC_ENT_SIZE_32;
        break;
      }
      case ELFhdr.ELFCLASS64: {
        relocation.r_offset = efile.readLongE();
        relocation.r_info = efile.readLongE();
        if (hasAddend)
          relocation.r_addend = efile.readLongE();
        relocation.relocationType = (int) (relocation.r_info & 0xffffffff);
        relocation.symbolTableIndex = (int) (relocation.r_info >> 32);
        off += RELOC_ENT_SIZE_64;
        break;
      }
      case ELFhdr.ELFCLASSNONE:
      default:
