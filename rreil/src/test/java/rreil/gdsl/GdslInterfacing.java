...
public class GdslInterfacing {
  ...
  private void comparePrint (boolean block, byte[] bytes) {
    // TODO: use GDSLX86_Disassembler class here to inherit the same settings for tests
    // move this test then to Binspot to fulfill the dependency relation
    X86Binder x86 = new X86Binder();

    System.out.println("Using frontend " + x86.getFrontend() + "...");
    Gdsl gdsl = new Gdsl(x86.getFrontend());
    ...
    StatementCollection statements = ((StatementCollectionBuilder) c).build().getResult();
...
//    X86Binder x86 = new X86Binder(frontends);
  ...
  @Test public void testBranch () {
    byte[] bytes = new byte[] {(byte) 0xe9, (byte) 0xa6, (byte)0x01, (byte)0x00, (byte)0x00};
  ...
  @Test public void testCall () {
    byte[] bytes = new byte[] {(byte) 0xe8, (byte) 0xae, (byte)0xff, (byte)0xff, (byte)0xff};
