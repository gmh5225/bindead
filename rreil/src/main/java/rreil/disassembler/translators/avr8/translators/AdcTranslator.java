...
public class AdcTranslator extends AVR8OperationTranslator {
  private static LowLevelRReilFactory factory = LowLevelRReilFactory.getInstance();

  public static class AdcFlagEmitter implements InsnEmitter {
    public static InsnEmitter $ = new AdcFlagEmitter();
    ...
    @Override public void emit (final TranslationCtx env, final LowLevelRReilOpnd dst, final LowLevelRReilOpnd src1, final LowLevelRReilOpnd src2, final List<LowLevelRReil> instructions) {
      ...
      instructions.add(factory.XOR(env.getNextReilAddress(), t2, dst.withOffset(3, 1), factory.immediate(t2.size(), (1 << t2.size()) - 1)));
      ...
      // LowLevelRReilOpnd t3 = env.temporaryRegister(1);
      //
      // instructions.addAll(Arrays.asList(factory.AND(env.getNextReilAddress(),
      // t1, src1.withOffset(7, 1), src2.withOffset(7, 1))));
      // instructions.addAll(Arrays.asList(factory.XOR(env.getNextReilAddress(),
      // t2, dst.withOffset(7, 1), factory.immediate(t2.size(), (1 <<
      // t2.size()) - 1))));
      // instructions.addAll(Arrays.asList(factory.AND(env.getNextReilAddress(),
      // t1, t1, t2)));
      //
      // instructions.addAll(Arrays.asList(factory.OR(env.getNextReilAddress(),
      // t3, src1.withOffset(7, 1), src2.withOffset(7, 1))));
      // instructions.addAll(Arrays.asList(factory.AND(env.getNextReilAddress(),
      // t3, t3, t2)));
      // instructions.addAll(Arrays.asList(factory.XOR(env.getNextReilAddress(),
      // t3, t3, factory.immediate(t3.size(), (1 << t3.size()) - 1))));
      //
      // instructions.addAll(Arrays.asList(factory.OR(env.getNextReilAddress(),
      // AVR8Helpers.V_OPERAND, t1, t3)));
      //
  ...
  public AdcTranslator () {
    super(ReturnType.Register);
  }

  @Override public void emit (final TranslationCtx env, final LowLevelRReilOpnd dst, final LowLevelRReilOpnd src1, final LowLevelRReilOpnd src2, final List<LowLevelRReil> instructions) {
    final LowLevelRReilOpnd t1 = env.temporaryRegister(dst.size());
    instructions.add(factory.CONVERT(env.getNextReilAddress(), t1, AVR8Helpers.C_OPERAND));
    instructions.add(factory.ADD(env.getNextReilAddress(), dst, src1, src2));
    instructions.add(factory.ADD(env.getNextReilAddress(), dst, dst, t1));
    AdcFlagEmitter.$.emit(env, dst, src1, src2, instructions);
  }
}
