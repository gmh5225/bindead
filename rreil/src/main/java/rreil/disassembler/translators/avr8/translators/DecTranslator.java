...
/**
 * Dec instruction emitter.
 *
 * @author mb0
 */
public class DecTranslator extends AVR8OperationTranslator {
  private static LowLevelRReilFactory factory = LowLevelRReilFactory.getInstance();

  public static class DecFlagEmitter implements InsnEmitter {
    public static InsnEmitter $ = new DecFlagEmitter();
    ...
    @Override public void emit (final TranslationCtx env, final LowLevelRReilOpnd dst, final LowLevelRReilOpnd src1, final LowLevelRReilOpnd src2, final List<LowLevelRReil> instructions) {
      ...
      instructions.add(factory.CMPEQ(env.getNextReilAddress(), AVR8Helpers.V_OPERAND, src1, factory.immediate(src1.size(), 0x80)));
  ...
  @Override public void emit (final TranslationCtx env, final LowLevelRReilOpnd dst, final LowLevelRReilOpnd src1, final LowLevelRReilOpnd src2, final List<LowLevelRReil> instructions) {
    instructions.add(factory.SUB(env.getNextReilAddress(), dst, src1, factory.immediate(src1.size(), 1)));

    DecFlagEmitter.$.emit(env, dst, src1, src2, instructions);
  }
}
