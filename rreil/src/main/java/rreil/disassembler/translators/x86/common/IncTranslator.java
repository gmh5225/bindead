...
public class IncTranslator implements InsnTranslator {
  private static LowLevelRReilFactory factory = LowLevelRReilFactory.getInstance();

  private static final InsnEmitter emitter = new X86FlagHelpers.IncFlagEmitter();
  ...
  public void translate (final TranslationCtx env, final Instruction instruction, final List<LowLevelRReil> instructions) {
    ...
    if (opnd.getOperandStack().size() > 1) {
      ...
      instructions.add(factory.ADD(env.getNextReilAddress(), t, src, one));
