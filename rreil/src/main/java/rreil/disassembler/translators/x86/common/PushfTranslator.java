...
public class PushfTranslator implements InsnTranslator {
  ...
  public void translate (final TranslationCtx env, final Instruction instruction, final List<LowLevelRReil> instructions) {
    ...
    final LowLevelRReilOpnd tmp = registerTranslator.temporaryRegister(env, env.getDefaultArchitectureSize());
    instructions.addAll(Arrays.asList(
        factory.MOV(env.getNextReilAddress(), tmp, factory.immediate(tmp.size(), 2)),
