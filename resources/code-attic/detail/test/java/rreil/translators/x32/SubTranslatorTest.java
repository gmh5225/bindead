...
public class SubTranslatorTest {
  ...
  @Test public void test001 () {
    // sub eax,0xffffffff
    final byte[] code = TstHelpers.pack(0x83, 0xe8, 0xff);
    ...
    assertEquals(BigInt.valueOf(2), ctx.get("eax"));
  ...
  @Test public void test002 () {
    // sub eax,0x80000000
    final byte[] code = TstHelpers.pack(0x2d, 0x00, 0x00, 0x00, 0x80);
    final InterpCtx ctx = x32.emptyInterpCtx();
    ...
    assertEquals(BigInt.valueOf("FFFFFFFF", 16), ctx.get("eax"));
    assertEquals(BigInt.ONE, ctx.getBool("CF"));
  ...
  @Test public void test003 () {
    // sub eax,1
    final byte[] code = TstHelpers.pack(0x83, 0xe8, 0x01);
    final InterpCtx ctx = x32.emptyInterpCtx();
    ...
    assertEquals(BigInt.valueOf("7FFFFFFF", 16), ctx.get("eax"));
  ...
  @Test public void test004 () {
    // sub eax,ebx
    final byte[] code = TstHelpers.pack(0x29, 0xd8);
    final InterpCtx ctx = x32.emptyInterpCtx();
    ...
    assertEquals(BigInt.valueOf("FFFFF000", 16), ctx.get("eax"));
  ...
  @Test public void test005 () {
    // sub eax,eax
    final byte[] code = TstHelpers.pack(0x29, 0xc0);
    final InterpCtx ctx = x32.emptyInterpCtx();
    ...
    assertEquals(BigInt.ZERO, ctx.get("eax"));
    assertEquals(BigInt.ZERO, ctx.getBool("CF"));
    assertEquals(BigInt.ZERO, ctx.getBool("OF"));
    assertEquals(BigInt.ZERO, ctx.getBool("SF"));
